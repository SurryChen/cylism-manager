...
系统应当根据站点元数据生成 NGINX server 块配置文件，并通过 Agent 部署到目标服务器。
...
系统应当解析目标服务器上已有的 NGINX 配置，并将发现的站点导入为非受管记录。
...
### Requirement: 站点接管
系统应当允许用户"接管"导入的（非受管）站点，将其转为受管站点。
