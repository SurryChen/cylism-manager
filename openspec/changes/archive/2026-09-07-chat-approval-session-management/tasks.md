...
- [x] 1.3 Add session-correlation propagation only when Nanobot provides trusted tool-invocation context.
...
- [x] 4.1 Run focused Go, Python, and Vue tests while implementing each task.
- [x] 4.2 Run `go test ./...`, `go build ./...`, `npm --prefix web test`, `npm --prefix web run build`, Python session tests, and `git diff --check`.
