...
- Registry 固定部署在 `cylism-system` 命名空间，并引用用户在该命名空间中预先创建的 `local-path` PVC。对于待绑定 PVC，用户选择的数据节点决定首次绑定位置；已绑定 PVC 则使用其绑定节点。
...
- 新增 Registry 数据模型、SQLite migration、REST API、Kubernetes Deployment/Service/Ingress/Secret 渲染和状态探测；PVC 由既有存储卷管理创建后选择引用。
