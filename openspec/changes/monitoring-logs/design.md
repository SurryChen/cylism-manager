...
每次请求必须使用预设范围或同时指定精确 UTC 起止时间，最大范围为 24 小时，最大返回 500 行，最大关键字长度为 256 字符，后端查询超时为 10 秒。接口返回时间戳、行文本和标签摘要，并带回已使用的范围与是否命中上限。所有筛选 ID 均按现有项目/环境关系校验，防止跨工作区枚举日志。

关键字保留单个自由文本的兼容行为，同时支持受限的双引号字符串、`\"`、`\\`、`\/` 转义及 `AND`/`OR`。`AND` 优先于 `OR`，首期不支持括号；后端将 OR 表达式拆为最多 8 个 AND 分支，每个分支构造连续的 Loki 行过滤，最多允许 16 个关键词。各分支共享同一 10 秒上下文，结果在平台端去重、按时间倒序合并后再截断，不能向浏览器开放原始 LogQL。
