...
describe('ManagedOCIRegistries view', () => {
  ...
  it('keeps infrastructure status inline and configuration in the edit modal without duplicating node mirror controls', async () => {
    ...
    const wrapper = mountRegistry()
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(wrapper.get('[data-testid="registry-overview-section"] h2').text()).toBe('运行概览')
    expect(wrapper.findAll('.registry-overview .metric')).toHaveLength(3)
    expect(wrapper.get('[data-testid="registry-overview-section"]').find('[data-testid="edit-registry"]').exists()).toBe(true)
  ...
  it('loads the first repository and its tags in the self-hosted Registry workspace without a nested tab', async () => {
    const { api } = await import('../api/index.js')
  ...
  it('selects another repository after deleting the active repository', async () => {
    ...
    api.get.mockImplementation(path => {
      ...
      if (path === '/managed-oci-registries/1/catalog') { catalogReads += 1; return Promise.resolve({ repositories: catalogReads === 1 ? ['team/orders', 'team/payments'] : ['team/payments'], next: '' }) }
      if (path === '/managed-oci-registries/1/catalog/tags?repository=team%2Forders') return Promise.resolve({ repository: 'team/orders', tags: [{ name: 'v1' }], next: '' })
      if (path === '/managed-oci-registries/1/catalog/tags?repository=team%2Fpayments') return Promise.resolve({ repository: 'team/payments', tags: [{ name: 'v2' }], next: '' })
    ...
    api.post.mockResolvedValue({ repository: 'team/orders', affected_tags: [], references: [] })
    api.delete.mockResolvedValue({})
    const wrapper = mountRegistry()
    await new Promise(resolve => setTimeout(resolve, 0))
    await new Promise(resolve => setTimeout(resolve, 0))

    await wrapper.get('[aria-label="删除仓库 team/orders"]').trigger('click')
    await new Promise(resolve => setTimeout(resolve, 0))
    document.body.querySelector('.registry-delete-modal .btn-danger').click()
    await new Promise(resolve => setTimeout(resolve, 0))
    await new Promise(resolve => setTimeout(resolve, 0))

    expect(wrapper.get('.registry-repository-row.is-selected').text()).toContain('team/payments')
    expect(wrapper.get('.registry-tags-panel').text()).toContain('v2')
    wrapper.unmount()
  })
  ...
  it('shows catalog failures as a compact retryable workspace state', async () => {
    ...
    expect(error.findComponent({ name: 'SurfaceCard' }).exists()).toBe(true)
    expect(error.classes()).toContain('surface-card')
