  ...
  <div>
    ...
    <div class="settings-grid">
      ...
      <section class="card temporary-token-card">
        ...
        <div class="settings-action-row"><button class="btn btn-sm btn-primary" :disabled="creatingTemporaryToken" @click="createTemporaryToken">{{ creatingTemporaryToken ? '生成中...' : '生成临时秘钥' }}</button></div>
        <div v-if="generatedTemporaryToken" class="secret-once temporary-token-secret"><strong>仅显示一次，请立即复制</strong><code>{{ generatedTemporaryToken }}</code><button class="btn btn-sm" @click="copyTemporaryToken">复制</button></div>
        <div v-if="temporaryTokens.length" class="temporary-token-list"><div v-for="item in temporaryTokens" :key="item.id" class="temporary-token-row"><div><strong>{{ item.label || '未命名秘钥' }}</strong><small>创建于 {{ formatDateTime(item.created_at) }} · 到期 {{ formatDateTime(item.expires_at) }}</small><span class="badge" :class="item.status === 'active' ? 'badge-online' : item.status === 'expired' ? 'badge-offline' : 'badge-danger'">{{ item.status === 'active' ? '生效中' : item.status === 'expired' ? '已过期' : '已撤销' }}</span></div><button v-if="item.status === 'active'" class="btn btn-sm btn-danger" :disabled="revokingTemporaryToken === item.id" @click="revokeTemporaryToken(item)">{{ revokingTemporaryToken === item.id ? '撤销中...' : '撤销' }}</button></div></div>
...
onMounted(async () => {
  await Promise.all([refresh({ syncForm: true }), loadCertificates(), loadTemporaryTokens()])
...
async function loadTemporaryTokens() { try { temporaryTokens.value = await api.get('/auth/temporary-tokens') } catch { temporaryTokens.value = [] } }
async function createTemporaryToken() {
  creatingTemporaryToken.value = true
  try { const result = await api.post('/auth/temporary-tokens', temporaryTokenForm.value); generatedTemporaryToken.value = result.token; temporaryTokenForm.value.label = ''; await loadTemporaryTokens() } finally { creatingTemporaryToken.value = false }
}
async function revokeTemporaryToken(item) {
  revokingTemporaryToken.value = item.id
  try { await api.delete(`/auth/temporary-tokens/${item.id}`); await loadTemporaryTokens() } finally { revokingTemporaryToken.value = 0 }
}
async function copyTemporaryToken() { if (generatedTemporaryToken.value) await navigator.clipboard?.writeText(generatedTemporaryToken.value) }
