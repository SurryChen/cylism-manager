...
describe('Node registry mirrors view', () => {
  ...
  it('keeps mirror table rows compact and exposes long values on hover', async () => {
    ...
    const name = wrapper.get('.mirror-name')
    const registry = wrapper.get('.mirror-registry')
    const endpoints = wrapper.get('.mirror-endpoints')
    expect(name.attributes('aria-label')).toContain('docker-hub-mirror')
    expect(name.attributes('aria-label')).toContain('docker.io/library/busybox:1.36')
    expect(registry.attributes('aria-label')).toBe('docker.io')
    expect(endpoints.attributes('aria-label')).toBe('https://docker.1panel.live')
    await endpoints.trigger('mouseenter', { clientX: 80, clientY: 120 })
    await settle()
    expect(document.body.querySelector('.mirror-value-tooltip')?.textContent).toBe('https://docker.1panel.live')
    await endpoints.trigger('mouseleave')
    expect(document.body.querySelector('.mirror-value-tooltip')).toBeNull()
    expect(wrapper.find('.mirror-rule-list small').exists()).toBe(false)
    ...
    await wrapper.get('.workspace-actions .btn-primary').trigger('click')
