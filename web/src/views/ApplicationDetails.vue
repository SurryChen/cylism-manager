<template>
  <div>
    ...
    <Teleport to="body"><div v-if="showTemplateEditor" class="overlay" @click.self="closeTemplateEditor"><div class="modal editor-modal"><h2 class="modal-title">{{ editingTemplate ? '编辑上线模板' : '新建上线模板' }}</h2><form @submit.prevent="saveTemplate">
      ...
      <div class="form-group"><label class="form-label">部署节点</label><select v-model="templateForm.node_name" class="form-select"><option value="">由 Kubernetes 调度</option><option v-for="node in clusterNodes" :key="node.name" :value="node.name">{{ node.display_name }} · hostname={{ node.name }}</option></select><p class="form-hint">选择后将使用 kubernetes.io/hostname 标签调度；本地 PVC 必须与其已绑定节点一致，首次挂载 local-path PVC 时将决定数据落盘节点。</p></div>
      <div class="form-group"><div class="volume-heading"><label class="form-label">数据卷挂载</label><button type="button" class="btn btn-sm" :disabled="!persistentVolumeClaims.length" @click="addVolume">+ 添加挂载</button></div><div v-if="templateForm.volumes.length" class="volume-list"><div v-for="(volume, index) in templateForm.volumes" :key="index" class="volume-row"><select v-model="volume.claim_name" class="form-select"><option v-for="claim in persistentVolumeClaims" :key="claim.name" :value="claim.name">{{ claim.name }} · {{ claim.phase || 'Pending' }}</option></select><input v-model.trim="volume.mount_path" class="form-input" required placeholder="/data" /><label class="check-row"><input v-model="volume.read_only" type="checkbox" /> 只读</label><button type="button" class="icon-button" title="移除数据卷挂载" @click="removeVolume(index)"><Trash2 :size="16" /></button></div></div><p v-else class="form-hint">未配置数据卷，应用将按无状态方式发布。</p></div>
      <div class="form-row"><div class="form-group"><label class="form-label">ConfigMap 环境变量</label><textarea v-model="templateForm.config_text" class="form-input form-textarea" placeholder="MEILI_ADDR=http://meilisearch:7700&#10;DATA_DIR=/data"></textarea><p class="form-hint">每行一个 KEY=VALUE，发布时生成应用专属 ConfigMap。</p></div><div class="form-group"><label class="form-label">Secret 环境变量</label><textarea v-model="templateForm.secrets_text" class="form-input form-textarea" placeholder="NEXTAUTH_SECRET=&#10;MEILI_MASTER_KEY="></textarea><p class="form-hint">Secret 明文只用于保存时加密，读取模板时只显示键名；编辑时留空会保留原值。</p></div></div>
