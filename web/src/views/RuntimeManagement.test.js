...
describe('RuntimeManagement', () => {
  ...
  it('lists runtime instances and exposes deployment actions', async () => {
    ...
    await wrapper.findAll('.runtime-detail .button').find(button => button.text() === '健康检查').trigger('click')
  ...
  it('requires two confirmations before deleting runtime data', async () => {
    ...
    await wrapper.findAll('.runtime-detail .button').find(button => button.text() === '卸载并删除数据').trigger('click')
