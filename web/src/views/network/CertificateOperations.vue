...
const operationResource = useAsyncResource(({ signal }, namespace, certificateName) => getCertificateOperations(namespace, certificateName, { signal }), [])
const operationKey = computed(() => `${route.params.namespace || ''}/${route.params.name || ''}`)
watch(operationKey, async () => {
  const requestKey = operationKey.value
  loaded.value = false
  error.value = ''
  const result = await operationResource.refresh(route.params.namespace, route.params.name)
  if (requestKey !== operationKey.value) return
  if (result) operations.value = Array.isArray(result) ? result : []
  else if (operationResource.error.value) error.value = operationResource.error.value.message || '加载签发过程失败'
  loaded.value = true
}, { immediate: true })
</script>
