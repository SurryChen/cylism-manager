...
function newReleaseForm() { return { registry_id: 0, image: '', container_port: 8080, replicas: 1, resources: { requests_cpu: '100m', requests_memory: '128Mi', limits_cpu: '500m', limits_memory: '512Mi' }, health: { readiness_path: '/healthz', liveness_path: '/healthz' }, service: { port: 80, target_port: 8080 }, endpoint: { exposure: 'cluster', domain: '', path: '/', tls_enabled: false, issuer_ref: '' } } }
...
async function openRelease(app) { error.value = ''; releaseForm.value = newReleaseForm(); try { releaseRegistries.value = await api.get(`/image-registries?project_id=${app.project_id}`) || []; const defaultRegistryID = app.project?.default_image_registry_id; if (releaseRegistries.value.some(registry => registry.id === defaultRegistryID && registry.enabled)) releaseForm.value.registry_id = defaultRegistryID } catch (e) { releaseRegistries.value = []; error.value = e.message || '加载可用镜像仓库失败' } finally { releaseApp.value = app } }
async function createRelease() { submitting.value = true; error.value = ''; try { const applicationID = releaseApp.value.id; const release = await api.post(`/applications/${applicationID}/releases`, releaseForm.value); releaseApp.value = null; await router.push(`/applications/${applicationID}/releases/${release.id}`); fetchApplications() } catch (e) { error.value = e.message || '创建发布失败' } finally { submitting.value = false } }
