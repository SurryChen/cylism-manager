    ...
    <template v-if="section === 'workspace'">
      ...
      <div v-else-if="applicationsLoaded" class="empty-state"><span class="empty-text">当前项目与环境下还没有应用</span></div></section>
        <div class="workspace-grid">
          ...
          <section class="workspace-section"><div class="section-heading"><div><h2>项目镜像仓库</h2><p>当前项目授权使用的镜像仓库</p></div><button class="btn btn-sm" @click="openRegistries">管理仓库</button></div><div v-if="workspaceRegistries.length" class="compact-list"><button v-for="registry in workspaceRegistries" :key="registry.id" class="list-row" @click="openRegistries"><span><strong>{{ registry.name }}</strong><small>{{ registry.endpoint }}</small></span><span class="badge" :class="registry.enabled ? 'badge-online' : 'badge-offline'">{{ registry.enabled ? '可用' : '停用' }}</span></button></div><div v-else class="empty-inline">当前项目还未授权镜像仓库</div></section>
        </div>
      </template>
    </template>
...
const unassignedDomains = computed(() => globalDomains.value.filter(domain => !domain.environment_id))
...
async function loadSection(section) { if (section === 'projects') { await Promise.all([fetchProjects(), fetchApplications(), fetchImageRegistries()]) } else if (section === 'overview') { await fetchReleaseHistory(); try { globalDomains.value = await api.get('/domains') || [] } catch (e) { error.value = e.message || '加载证书概览失败' } } else { await fetchProjects(); if (!workspaceProjectID.value) { const saved = JSON.parse(localStorage.getItem('cylism.application-workspace') || '{}'); const savedProject = projects.value.find(project => project.id === Number(saved.project_id)); const savedEnvironment = savedProject?.environments?.find(environment => environment.id === Number(saved.environment_id)); if (savedProject && savedEnvironment) { await updateWorkspace(savedProject.id, savedEnvironment.id); return } } if (workspaceReady.value) await fetchWorkspace(); else { workspaceOverview.value = null; workspaceRegistries.value = []; applications.value = []; applicationsLoaded.value = true } } }
...
async function createRelease() { error.value = ''; if (releaseForm.value.endpoint.exposure === 'public' && !releaseForm.value.endpoint.domain_id) { error.value = '请选择当前环境中证书已就绪的受管域名'; return } submitting.value = true; try { const applicationID = releaseApp.value.id; const release = await api.post(`/applications/${applicationID}/releases`, releasePayload()); releaseApp.value = null; await router.push(`/applications/${applicationID}/releases/${release.id}`); fetchWorkspace() } catch (e) { error.value = e.message || '创建发布失败' } finally { submitting.value = false } }
