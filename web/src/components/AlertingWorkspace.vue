<template>
  ...
  <Teleport to="body"><div v-if="settingsOpen" class="overlay alert-settings-overlay" @click.self="settingsOpen = false"><form class="modal alert-settings-modal" @submit.prevent="saveSettings"><header class="drawer-header"><div><h2>告警设置</h2><p>规则与通知渠道</p></div><button class="icon-button" type="button" title="关闭" aria-label="关闭" @click="settingsOpen = false"><X :size="17" /></button></header><section class="drawer-section"><div class="drawer-section-heading"><div><h3>飞书通知</h3><p>地址仅写入 Secret，不会再次展示</p></div><span class="badge" :class="status?.feishu_configured ? 'badge-online' : 'badge-offline'">{{ status?.feishu_configured ? '已配置' : '未配置' }}</span></div><input v-model.trim="settingsForm.feishu_webhook_url" class="form-input" type="url" placeholder="填写新的飞书机器人地址以更新" /><div class="drawer-actions"><button class="btn" type="button" :disabled="testingChannel !== '' || !status?.feishu_configured" @click="testNotification('feishu')">{{ testingChannel === 'feishu' ? '发送中...' : '测试飞书通知' }}</button></div></section><section class="drawer-section"><div class="drawer-section-heading"><div><h3>邮件通知</h3><p>通过 SMTP 发送，可与飞书并行</p></div><span class="badge" :class="status?.email_configured ? 'badge-online' : 'badge-offline'">{{ status?.email_configured ? '已配置' : '未配置' }}</span></div><label class="check-row"><input v-model="settingsForm.email.enabled" type="checkbox" /> 配置 SMTP 邮件通知</label><div v-if="settingsForm.email.enabled" class="email-fields"><div class="form-row"><div class="form-group"><label class="form-label">SMTP 主机</label><input v-model.trim="settingsForm.email.smtp_host" class="form-input" required placeholder="smtp.example.com" /></div><div class="form-group"><label class="form-label">端口</label><input v-model.number="settingsForm.email.smtp_port" class="form-input" type="number" min="1" max="65535" required /></div></div><div class="form-row"><div class="form-group"><label class="form-label">TLS 模式</label><select v-model="settingsForm.email.tls_mode" class="form-select"><option value="starttls">STARTTLS (587)</option><option value="tls">TLS (465)</option></select></div><div class="form-group"><label class="form-label">SMTP 用户名</label><input v-model.trim="settingsForm.email.username" class="form-input" /></div></div><div class="form-group"><label class="form-label">SMTP 密码</label><input v-model="settingsForm.email.password" class="form-input" type="password" /><p class="form-hint">账号认证可留空；填写用户名时必须同时填写密码。</p></div><div class="form-row"><div class="form-group"><label class="form-label">发件人</label><input v-model.trim="settingsForm.email.from" class="form-input" type="email" required placeholder="alerts@example.com" /></div><div class="form-group"><label class="form-label">收件人</label><input v-model.trim="settingsForm.email.to" class="form-input" required placeholder="ops@example.com, admin@example.com" /></div></div></div><div class="drawer-actions"><button class="btn" type="button" :disabled="testingChannel !== '' || !status?.email_configured" @click="testNotification('email')">{{ testingChannel === 'email' ? '发送中...' : '测试邮件通知' }}</button></div></section><section class="drawer-section"><div class="drawer-section-heading"><div><h3>告警规则</h3><p>修改后将重载规则评估</p></div></div><div class="rule-list"><div v-for="rule in settingsForm.rules" :key="rule.id" class="rule-row"><label class="rule-title"><input v-model="rule.enabled" type="checkbox" /><span>{{ rule.name }}</span></label><div class="rule-fields"><label v-if="rule.threshold > 0"><span>阈值</span><input v-model.number="rule.threshold" class="form-input" type="number" min="0" max="100000" /></label><label><span>持续</span><input v-model.number="rule.duration_minutes" class="form-input" type="number" min="1" max="1440" /></label><small>分钟</small></div></div></div></section><footer class="drawer-footer"><button class="btn" type="button" :disabled="saving" @click="settingsOpen = false">取消</button><button class="btn btn-primary" :disabled="saving">{{ saving ? '保存中...' : '保存设置' }}</button></footer></form></div></Teleport>
</template>
...
const settingsForm = ref({ feishu_webhook_url: '', email: blankEmail(), rules: [] })
...
async function openSettings() {
  settingsForm.value = { feishu_webhook_url: '', email: blankEmail(), rules: (status.value?.rules || []).map(rule => ({ ...rule })) }
