  ...
  <template v-else-if="status">
    ...
    <template v-else>
      ...
      <section class="automation-panel section-gap">
        <div class="automation-toggle"><div><strong>启用告警自动分析</strong><small>只开放固定的告警、指标和节点诊断接口</small></div><label class="switch"><input v-model="automationPolicy.enabled" type="checkbox" /><span /></label></div>
      ...
      <section v-if="automationEvents.length" class="automation-events section-gap"><div class="alert-section-heading"><div><h2>自动处置事件</h2><p>诊断、审批、执行与复测状态以持久化事件为准。</p></div></div><div class="event-list"><article v-for="event in automationEvents" :key="event.id" class="event-row"><div class="event-state" :class="`state-${event.status}`"><Bot :size="16" /></div><div class="event-copy"><strong>{{ event.alert_name }}<span v-if="event.node_name"> · {{ event.node_name }}</span></strong><small>{{ automationEventState(event) }} · {{ formatTime(event.updated_at) }}</small><p v-if="event.diagnostic_summary">{{ event.diagnostic_summary }}</p><p v-else-if="event.last_error" class="event-error">{{ event.last_error }}</p><details v-if="event.report"><summary>查看 Agent 报告</summary><pre>{{ event.report }}</pre></details></div><span v-if="event.operation_id" class="badge badge-deploying">等待审批</span></article></div></section>
...
async function saveAutomationPolicy() {
  ...
  try { await api.put('/monitoring/alerts/automation-policy', automationPolicy.value); await refresh() } finally { automationSaving.value = false }
}
