<template>
  ...
  <template v-else-if="status">
    ...
    <section v-if="status.state !== 'ready'" class="card logging-wait section-gap"><div class="empty-state"><span class="empty-icon">◌</span><span class="empty-text">{{ status.message || '等待 Loki 与 Alloy 工作负载就绪' }}</span></div><div class="modal-actions status-actions"><button class="btn" :disabled="loading" @click="refresh">重新检测</button></div></section>

    <template v-else>
      <section class="logging-section-heading section-gap"><div><h2>日志检索</h2><p>按容器标准输出检索。日志不会在打开页面时自动加载。</p></div><div class="icon-actions"><button class="icon-button" title="日志设置" aria-label="日志设置" @click="openSettings"><Settings2 :size="16" /></button><button class="icon-button" title="刷新日志状态与筛选项" aria-label="刷新日志状态与筛选项" :disabled="loading" @click="refresh"><RefreshCw :size="16" :class="{ 'is-spinning': loading }" /></button></div></section>
      ...
      <form class="card logging-query" @submit.prevent="queryLogs">
        ...
        <div class="logging-search-row"><input v-model.trim="queryForm.keyword" class="form-input" maxlength="256" placeholder="按日志内容筛选，例如 error 或 connection refused" /><button class="btn btn-primary" data-testid="query-logs" :disabled="querying" type="submit">{{ querying ? '查询中...' : '查询日志' }}</button></div>
      </form>
