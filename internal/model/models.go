...
const (
	AssistantRuntimeMigrationPending      = "pending"
	AssistantRuntimeMigrationProvisioning = "provisioning_target"
	AssistantRuntimeMigrationStopping     = "stopping_source"
	AssistantRuntimeMigrationCopying      = "copying"
	AssistantRuntimeMigrationVerifying    = "verifying"
	AssistantRuntimeMigrationStarting     = "starting_target"
	AssistantRuntimeMigrationCleaning     = "cleaning_legacy"
	AssistantRuntimeMigrationSucceeded    = "succeeded"
	AssistantRuntimeMigrationFailed       = "failed"
)

// AssistantRuntimeMigration persists the one-time cross-namespace move of
// the managed Runtime audit store. It intentionally contains no API secret.
type AssistantRuntimeMigration struct {
	ID               uint       `gorm:"primaryKey" json:"id"`
	ProviderID       uint       `gorm:"index;not null" json:"provider_id"`
	SourceNamespace  string     `gorm:"size:128;not null" json:"source_namespace"`
	SourcePVCName    string     `gorm:"size:253;not null" json:"source_pvc_name"`
	SourceNodeName   string     `gorm:"size:253" json:"source_node_name"`
	SourceReplicas   int32      `json:"source_replicas"`
	TargetNamespace  string     `gorm:"size:128;not null" json:"target_namespace"`
	...
	Storage          string     `gorm:"size:64;not null" json:"storage"`
	StorageClassName string     `gorm:"size:253" json:"storage_class_name,omitempty"`
...
func IsAssistantRuntimeMigrationTerminal(status string) bool {
	return status == AssistantRuntimeMigrationSucceeded || status == AssistantRuntimeMigrationFailed
}
