...
func TestMaintenanceDiskInspectRequiresExplicitGrantAndRedactsFailures(t *testing.T) {
	...
	called := 0
	handler := newTestAgentHandler(s, nil, agentAuthenticatorStub{instance: instance}).WithMaintenanceInspector(func(_ context.Context, server *model.Server) (agentDiskInspection, error) {
		called++
	...
	failing := newTestAgentHandler(s, nil, agentAuthenticatorStub{instance: instance}).WithMaintenanceInspector(func(context.Context, *model.Server) (agentDiskInspection, error) {
