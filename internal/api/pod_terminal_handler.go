...
func (h *K8sHandler) PodTerminal(c *gin.Context) {
	...
	pod, err := K8s.Clientset.CoreV1().Pods(namespace).Get(K8s.Ctx(), name, metav1.GetOptions{})
	if err != nil {
	...
	request := K8s.Clientset.CoreV1().RESTClient().Post().
		Resource("pods").
		Namespace(namespace).
		Name(name).
		SubResource("exec").
	...
	executor, err := remotecommand.NewSPDYExecutor(K8s.Config, http.MethodPost, request.URL())
	if err != nil {
