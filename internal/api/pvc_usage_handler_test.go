...
func TestPersistentVolumeClaimUsageReadsBoundLocalVolume(t *testing.T) {
	...
	router := gin.New()
	router.GET("/api/k8s/persistent-volume-claims/usage", NewK8sHandler(st).ListPersistentVolumeClaimUsage)
...
func TestPersistentVolumeClaimUsageReportsUnsupportedVolume(t *testing.T) {
	originalK8s := K8s
	K8s = &k8sclient.Client{Clientset: k8sfake.NewSimpleClientset(&corev1.PersistentVolumeClaim{ObjectMeta: metav1.ObjectMeta{Name: "remote-data", Namespace: "default"}})}
	defer func() { K8s = originalK8s }()

	router := gin.New()
	router.GET("/api/k8s/persistent-volume-claims/usage", NewK8sHandler().ListPersistentVolumeClaimUsage)
	response := httptest.NewRecorder()
	router.ServeHTTP(response, httptest.NewRequest(http.MethodGet, "/api/k8s/persistent-volume-claims/usage", nil))
