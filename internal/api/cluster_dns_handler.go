...
type ClusterDNSHandler struct{ store clusterDNSStore }

func NewClusterDNSHandler(s clusterDNSStore) *ClusterDNSHandler { return &ClusterDNSHandler{store: s} }
...
func (h *ClusterDNSHandler) Status(c *gin.Context) {
	...
	configMap, err := K8s.Clientset.CoreV1().ConfigMaps(coreDNSNamespace).Get(c.Request.Context(), coreDNSConfigMap, metav1.GetOptions{})
	if err != nil {
	...
	pods, err := K8s.Clientset.CoreV1().Pods(coreDNSNamespace).List(c.Request.Context(), metav1.ListOptions{LabelSelector: "k8s-app=kube-dns"})
	if err != nil {
