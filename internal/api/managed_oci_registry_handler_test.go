...
func setupManagedOCIRegistryRouterWithHandler(t *testing.T) (*gin.Engine, *store.Store, *ManagedOCIRegistryHandler) {
	...
	original := K8s
	K8s = &k8s.Client{Clientset: k8sfake.NewSimpleClientset()}
...
func managedRegistryPayload() gin.H {
	return gin.H{
		...
		"registry_image": "registry:2.8", "data_node": "node-a", "data_path": "/data/cylism-registry",
		"tls_secret_name": "registry-tls", "pull_username": "cylism-pull", "pull_password": "safe-registry-password",
	}
}
...
func TestManagedOCIRegistryCreateRendersProtectedResources(t *testing.T) {
	...
	assertManagedRegistryDeployment(t, deployment, registry.DataNode, registry.DataPath)
...
func assertManagedRegistryDeployment(t *testing.T, deployment *appsv1.Deployment, node, path string) {
	t.Helper()
	if deployment.Spec.Strategy.Type != appsv1.RecreateDeploymentStrategyType || deployment.Spec.Template.Spec.NodeSelector[corev1.LabelHostname] != node {
		t.Fatalf("expected Recreate and node pinning: %#v", deployment.Spec)
	}
	if len(deployment.Spec.Template.Spec.Volumes) != 2 || deployment.Spec.Template.Spec.Volumes[0].HostPath == nil || deployment.Spec.Template.Spec.Volumes[0].HostPath.Path != path {
		t.Fatalf("expected hostPath storage and auth volume: %#v", deployment.Spec.Template.Spec.Volumes)
	}
	if deployment.Labels["app.kubernetes.io/managed-by"] != "cylism-manager" || deployment.Spec.Template.Spec.Containers[0].Image != "registry:2.8" {
