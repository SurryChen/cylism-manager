...
type KubernetesAdapter interface {
	...
	NamespaceClient
	CertificateReader
	SyncApplicationEndpoints(context.Context, application.ApplicationContext, []model.ApplicationEndpoint, int32) error
	InspectReleasePods(context.Context, application.ApplicationContext) (*model.ReleaseRuntime, error)
	ValidatePersistentVolumeClaims(context.Context, application.ApplicationContext, application.ReleaseSpec) error
	MigrateWorkloadKind(context.Context, application.ApplicationContext, application.ReleaseSpec, string) error
	KubernetesAvailable() bool
}

type kubernetesDependencies struct {
	applier      application.ResourceApplier
...
func (d *kubernetesDependencies) VerifyImage(ctx context.Context, spec application.ReleaseSpec) error {
	return d.applier.VerifyImage(ctx, spec)
}
func (d *kubernetesDependencies) Preflight(ctx context.Context, app application.ApplicationContext, spec application.ReleaseSpec) error {
	return d.applier.Preflight(ctx, app, spec)
}
func (d *kubernetesDependencies) Apply(ctx context.Context, resources *application.RenderedResources) error {
	return d.applier.Apply(ctx, resources)
}
func (d *kubernetesDependencies) WaitReady(ctx context.Context, app application.ApplicationContext, spec application.ReleaseSpec) error {
...
func (d *kubernetesDependencies) SyncApplicationEndpoints(ctx context.Context, app application.ApplicationContext, endpoints []model.ApplicationEndpoint, port int32) error {
	return d.endpoints.SyncApplicationEndpoints(ctx, app, endpoints, port)
}
func (d *kubernetesDependencies) InspectReleasePods(ctx context.Context, app application.ApplicationContext) (*model.ReleaseRuntime, error) {
	return d.inspector.InspectReleasePods(ctx, app)
}
func (d *kubernetesDependencies) ValidatePersistentVolumeClaims(ctx context.Context, app application.ApplicationContext, spec application.ReleaseSpec) error {
	return d.pvc.ValidatePersistentVolumeClaims(ctx, app, spec)
}
func (d *kubernetesDependencies) MigrateWorkloadKind(ctx context.Context, app application.ApplicationContext, spec application.ReleaseSpec, target string) error {
...
func NewKubernetesAdapter(client *k8sclient.Client) KubernetesAdapter {
	...
	applier := application.NewKubernetesApplier(client)
