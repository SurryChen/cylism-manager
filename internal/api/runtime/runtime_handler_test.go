...
func setupRuntimeRouter() (*gin.Engine, *store.Store) {
	...
	r := gin.New()
	h := NewRuntimeHandlerWithDependencies(s, []byte("01234567890123456789012345678901"), nil, runtime.BuiltinRegistry())
...
func TestRuntimeHandlerGeneratesAndHidesRuntimeAPICredentialOnDeploy(t *testing.T) {
	...
	r := gin.New()
	h := NewRuntimeHandlerWithDependencies(s, []byte("01234567890123456789012345678901"), manager, runtime.BuiltinRegistry())
	r.POST("/api/runtimes", h.Create)
