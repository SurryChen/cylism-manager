...
func ProbeSSHContext(ctx context.Context, server *model.Server, encKey []byte) (bool, string) {
	...
	out, err := transport.SSHExecServerContext(ctx, transport.SSHTimeout, server, encKey, "echo ok")
	if err != nil {
...
func RunPrechecksContext(ctx context.Context, server *model.Server, encKey []byte) []cluster.Precheck {
	...
	out, err = transport.SSHExecServerContext(ctx, transport.SSHTimeout, server, encKey, "id -u")
	rootOK := err == nil && strings.TrimSpace(string(out)) == "0"
	rootDetail := strings.TrimSpace(string(out))
	if !rootOK {
		sudoOut, sudoErr := transport.SSHExecServerContext(ctx, transport.SSHTimeout, server, encKey, "sudo -n true 2>&1")
	...
	out, err = transport.SSHExecServerContext(ctx, transport.SSHTimeout, server, encKey, "swapon --show 2>/dev/null | wc -l")
	...
	out, err = transport.SSHExecServerContext(ctx, transport.SSHTimeout, server, encKey, "uname -m")
	...
	out, err = transport.SSHExecServerContext(ctx, transport.SSHTimeout, server, encKey, "df -BG / | tail -1 | awk '{print $4}' | sed 's/G//'")
	diskGB := 0
	if err == nil {
...
func (i ServerInspector) Hostname(ctx context.Context, server *model.Server) (string, error) {
	if _, err := transport.SSHExecServerContext(ctx, transport.SSHTimeout, server, i.EncKey, "echo ok"); err != nil {
		return "", err
	}
	out, err := transport.SSHExecServerContext(ctx, transport.SSHTimeout, server, i.EncKey, "hostname")
	if err != nil {
		return "", err
	}
