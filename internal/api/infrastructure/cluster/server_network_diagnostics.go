...
func (h *ServerNetworkDiagnosticsHandler) collectNetworkLink(ctx context.Context, source *model.Server, targetIP string) (tailnetLinkDiagnostic, error) {
	...
	args := transport.BuildSSHArgs(source, h.encKey, source.Host)
	out, err := transport.SSHExecContext(ctx, 20*time.Second, append(args, "tailscale ping --c=3 --timeout=5s "+targetIP))
