...
func RegisterRoutes(r *gin.Engine, s *store.Store, encKey []byte, authCfg *AuthConfig) {
	...
	applications := apiGroup.Group("/applications")
	{
		...
		applications.GET("/:id/endpoint", applicationHandler.GetApplicationEndpoint)
		applications.PUT("/:id/endpoint", applicationHandler.UpdateApplicationEndpoint)
		applications.DELETE("/:id/endpoint", applicationHandler.DeleteApplicationEndpoint)
