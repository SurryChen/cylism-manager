...
type fakeSystemComponentAdapter struct{ client *k8sfake.Clientset }
...
func TestSystemComponentHandlerUsesInjectedAdapterForNodeValidation(t *testing.T) {
	fake := fakeSystemComponentAdapter{client: k8sfake.NewSimpleClientset()}
	h := (&SystemComponentHandler{}).WithAdapter(fake)
	if h.adapter == nil || h.listService == nil || h.service == nil {
		t.Fatal("system component adapter was not injected")
	}
}
