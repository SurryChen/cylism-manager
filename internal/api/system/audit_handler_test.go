...
func TestAuditHandlerListsLogsWithFiltersAndPagination(t *testing.T) {
	...
	r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/api/audit-logs?resource_type=application&action=deploy&limit=10&offset=0", nil))
	if w.Code != http.StatusOK || !strings.Contains(w.Body.String(), `"total":1`) || !strings.Contains(w.Body.String(), `"resource_type":"application"`) {
