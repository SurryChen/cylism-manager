...
实施状态：已完成（2026-08-30）。目录、复杂度和 Router 路由基线记录在 `docs/archive/architecture/internal-architecture-stage0-baseline.md`；共享 API 辅助函数已收敛到 `internal/api/shared`，现有 REST 路径、鉴权和业务行为保持不变。
...
internal/k8s/managed_registry.go
```
...
- 镜像代理的 Kubernetes Deployment、Service、节点检查、缓存清理和状态读取收敛至 `internal/k8s/node_registry_proxy.go`，K8s 层只接收服务层解密后的运行环境变量。
- Registry Proxy 与节点镜像源的 HTTP Adapter 已迁移至 `internal/api/delivery/`；路由、鉴权、错误码和响应字段保持不变。
- Registry Proxy 的 Ready Pod 查询、固定诊断命令、Pod exec 和诊断结果解析已迁移至 `internal/k8s/node_registry_proxy.go`；API 层不再执行 Kubernetes Pod exec。
